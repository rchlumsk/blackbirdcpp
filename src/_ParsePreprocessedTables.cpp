  ...
  while (!end_of_file)
  {
    ...
    */
    ...
    {/*:RedirectToFile*/
      ...
      filename = CorrectForRelativePath(filename, pOptions->bbp_filename);

      INPUT2.open(filename.c_str());
      if (INPUT2.fail()) {
        std::string warn = "ParsePreprocessedTablesFile: RedirectToFile: Cannot find file " + filename;
        ExitGracefully(warn.c_str(), BAD_DATA);
      }
      else {
        if (pMainParser != NULL) {
          ExitGracefully("ParsePreprocessedTablesFile::nested :RedirectToFile commands (in already redirected files) are not allowed.", BAD_DATA);
        }
