...
//////////////////////////////////////////////////////////////////
/// reads calendar from "calendar" attribute of NetCDF, converts to Blackbird enumerated type
//
/// \param ncid [in] netCDF file ID
/// \param varid_t [in] netCDF time variable ID
/// \param filename [in] filename of netCDF file
/// \param in_calendar [in] calendar type from Options object
/// \param noisy [in] boolean indicating a noisy run
/// \return calendar (PROLEPTIC_GREGORIAN if calendar not in file)
//
int GetCalendarFromNetCDF(const int ncid, int varid_t, const std::string filename, const enum_cd_type in_calendar, const bool noisy)
{
  ...
  if (retval != NC_ENOTATT)
  {
    ...
    if (calendar != in_calendar) {
      if ((calendar == CALENDAR_GREGORIAN) && (in_calendar == CALENDAR_PROLEPTIC_GREGORIAN)) {} //basically the same, not worth warning
      else {
        string warn;
        warn = "[Critical]: NetCDF file " + filename + " does not use same calendar as the simulation. Use the :Calendar command to ensure consistency";//TODO
...
//
void GetTimeInfoFromNetCDF(const char* unit_t, int calendar, const double* time, const int ntime, const std::string filename, double& tstep, double& start_day, int& start_yr, double& time_zone)
{
#ifdef _BBNETCDF_
  // -------------------------------
  ...
  //----------------------------
  std::string unit_t_str = to_string(unit_t);
...
//
void GetJulianDateFromNetCDFTime(const std::string unit_t_str, const int calendar, const double& time, double& julian_day, int& year)
{
#ifdef _BBNETCDF_
