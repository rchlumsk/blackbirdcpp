...
static CModel* pModel = NULL;  // the pModel is a global variable
...
inline void FinalizeGracefully(const char* statement, exitcode code)
{
  ...
  if (pModel == NULL) {
    std::cout << "============== Early Gracefully Exit ==============" << std::endl;
    std::cout << "Error Statement: " << statement << std::endl;
  ...
  if (code != BLACKBIRD_OPEN_ERR) { //avoids recursion problems
    ...
    else { WARNINGS << "SIMULATION COMPLETE :)" << std::endl; }
    WARNINGS.close();
  }
  ...
  std::cout << "Exiting Gracefully: " << statement << std::endl;
  std::cout << typeline << std::endl;
  std::cout << "============================================================" << std::endl;
